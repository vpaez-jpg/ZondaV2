// POST /api/generar-propuesta-pp
//
// Crea un trámite PP para un nuevo cliente (o cliente existente) y envía
...
import { enviarPropuestaPPEmail, type DatosPropuestaPP } from '@/lib/propuesta-pp'
...
export async function POST(req: NextRequest) {
  ...
  if (existingUser) {
    ...
    if (authError) {
      console.error('crear usuario PP:', authError)
      return NextResponse.json({ error: authError.message }, { status: 422 })
    }
    clienteId = authData.user.id
  }

  // ── 2. Crear el trámite PP ─────────────────────────────────
  const datosPropuesta: DatosPropuestaPP = {
    precio_24hs,
    precio_3dias,
    precio_5dias,
    ofrece_reunion,
  ...
  if (tramiteError || !tramite) {
    console.error('crear tramite PP:', tramiteError)
    return NextResponse.json({ error: 'Error creando el trámite PP' }, { status: 500 })
  }

  // ── 3. Enviar email con propuesta ──────────────────────────
  const { enviado, error: emailError } = await enviarPropuestaPPEmail({
  ...
  if (emailError) {
    console.warn('PP email warning:', emailError)
  }
