// POST /api/generar-propuesta-nda
//
// Crea un trámite NDA para un nuevo cliente (o cliente existente) y envía
// el email de bienvenida con la propuesta de precios.
//
// body: {
//   nombre:          string       — nombre del cliente
//   email:           string       — email del cliente
//   telefono?:       string       — teléfono del cliente (para WhatsApp)
//   password:        string       — contraseña temporal generada por el partner
//   precio_24hs:     number
//   precio_3dias:    number
//   precio_5dias:    number
...
import { enviarPropuestaNDAEmail, type DatosPropuestaNDA } from '@/lib/propuesta-nda'
...
export async function POST(req: NextRequest) {
  ...
  if (existingUser) {
    // Cliente ya existe — actualizamos su contraseña
    ...
    if (authError) {
      console.error('crear usuario NDA:', authError)
      return NextResponse.json({ error: authError.message }, { status: 422 })
    }
    clienteId = authData.user.id
  }

  // ── 2. Crear el trámite NDA ────────────────────────────────
  const datosPropuesta: DatosPropuestaNDA = {
    precio_24hs,
    precio_3dias,
    precio_5dias,
    ofrece_reunion,
  ...
  if (tramiteError || !tramite) {
    console.error('crear tramite NDA:', tramiteError)
    return NextResponse.json({ error: 'Error creando el trámite NDA' }, { status: 500 })
  }

  // ── 3. Enviar email con propuesta ──────────────────────────
  const { enviado, error: emailError } = await enviarPropuestaNDAEmail({
  ...
  if (emailError) {
    console.warn('NDA email warning:', emailError)
  }
