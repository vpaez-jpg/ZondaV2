...
// ── Email HTML ─────────────────────────────────────────────────
export function buildEmailHTMLTYC(
  datos: DatosPropuestaTYC,
...
<div class="wrapper">
  <div class="header">
    <h1>Propuesta de Términos y Condiciones</h1>
  ...
  <div class="body">
    ...
    <p style="font-size:14px;color:#6B7280;margin-top:4px;">
      ${partnerNombre} te comparte una propuesta para la redacción de los Términos y Condiciones de tu plataforma, adaptados a la legislación argentina y a las características específicas de tu negocio.
    </p>
    ...
    <div class="highlight">
      <div class="pricing-row">
        <div>
          ...
          <div class="tier-sub">Recibís los Términos y Condiciones el día hábil siguiente</div>
        </div>
      ...
      <div class="pricing-row" style="border-top:1px dashed #E2E8F0;margin-top:4px;padding-top:12px;">
        <div>
          ...
          <div class="tier-sub">Podés solicitar una reunión para revisar los Términos con tu abogado</div>
        </div>
    ...
    <p style="font-size:13px;color:#6B7280;">
      El precio incluye la redacción completa de los Términos y Condiciones a medida, según la información que nos brindes, una revisión y ajustes menores. El documento se entrega en formato Word y PDF.
    </p>

    <div class="section-title">Cómo avanzar</div>
    <p style="font-size:14px;color:#374151;">
      Para continuar, ingresá al portal de clientes y completá el cuestionario con los datos de tu empresa y las características de tu plataforma. Con esa información redactamos tus Términos y Condiciones.
    </p>
    ...
    <div class="note">
      El pago se coordina directamente con ${partnerNombre}. Una vez que completes el cuestionario, recibirás los Términos y Condiciones en el plazo seleccionado.
    </div>
  </div>
  <div class="footer">
...
// ── Envío via Resend ───────────────────────────────────────────
export async function enviarPropuestaTYCEmail(opts: {
  datos: DatosPropuestaTYC
  clienteNombre:   string
  clienteEmail:    string
  clientePassword: string
  ...
  if (!apiKey) {
    console.warn('propuesta-tyc: RESEND_API_KEY no configurada, email no enviado')
    return { enviado: false, error: 'API key no configurada' }
  }

  const html = buildEmailHTMLTYC(
    opts.datos,
    opts.clienteNombre,
    opts.clienteEmail,
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      subject: `Tu propuesta de Términos y Condiciones — ${opts.partnerNombre}`,
      html,
    }),
  })
  ...
  if (!res.ok) {
    ...
    console.error('Resend error TYC:', body)
