...
// ── Email HTML ─────────────────────────────────────────────────
export function buildEmailHTMLNDA(
  datos: DatosPropuestaNDA,
...
<div class="wrapper">
  <div class="header">
    <h1>Propuesta de Acuerdo de Confidencialidad (NDA)</h1>
  ...
  <div class="body">
    ...
    <p style="font-size:14px;color:#6B7280;margin-top:4px;">
      ${partnerNombre} te comparte una propuesta para la redacción de un Acuerdo de Confidencialidad (NDA) a tu medida, redactado conforme a la legislación argentina.
    </p>
    ...
    <div class="highlight">
      <div class="pricing-row">
        <div>
          ...
          <div class="tier-sub">Recibís el NDA el día hábil siguiente</div>
        </div>
      ...
      <div class="pricing-row" style="border-top:1px dashed #E2E8F0;margin-top:4px;padding-top:12px;">
        <div>
          ...
          <div class="tier-sub">Podés solicitar una reunión con tu abogado para que te explique el NDA</div>
        </div>
    ...
    <p style="font-size:13px;color:#6B7280;">
      El precio incluye la redacción completa del acuerdo según la información que nos brindes, una revisión y ajustes menores. El documento se entrega en formato Word y PDF firmado digitalmente.
    </p>

    <div class="section-title">Cómo avanzar</div>
    <p style="font-size:14px;color:#374151;">
      Para continuar, ingresá al portal de clientes y completá el cuestionario con los datos de las partes y las características del acuerdo. Con esa información redactamos tu NDA.
    </p>
    ...
    <div class="note">
      El pago se coordina directamente con ${partnerNombre}. Una vez que completes el cuestionario, recibirás el NDA en el plazo seleccionado.
    </div>
  </div>
  <div class="footer">
...
// ── Envío via Resend ───────────────────────────────────────────
export async function enviarPropuestaNDAEmail(opts: {
  datos: DatosPropuestaNDA
  clienteNombre:   string
  clienteEmail:    string
  clientePassword: string
  ...
  if (!apiKey) {
    console.warn('propuesta-nda: RESEND_API_KEY no configurada, email no enviado')
    return { enviado: false, error: 'API key no configurada' }
  }

  const html = buildEmailHTMLNDA(
    opts.datos,
    opts.clienteNombre,
    opts.clienteEmail,
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      subject: `Tu propuesta de NDA — ${opts.partnerNombre}`,
      html,
    }),
  })
  ...
  if (!res.ok) {
    ...
    console.error('Resend error NDA:', body)
